...
- **The TUI now shows every package an image pulls in, including deep
  transitive dependencies.** Library packages reached only through another
  package's runtime dependencies (for example `libexpat` via `python3`) were
  sometimes missing from the `in:<image>` view and from a unit's "pulls in"
  tree, even though the image installs them. The unit list and dependency trees
  now reflect the full closure.
