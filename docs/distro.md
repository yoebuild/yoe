...
Feed-materialized units (from `alpine_feed` / `debian_feed`) always carry a hard
...
| Distro | Status       | Release cadence                                                                                                                          | When it's the right choice                                                                                                                                                                                                                                                                                                                                                                                        |
| ------ | ------------ | --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Alpine | Production   | New stable branch ~every 6 months; ~2-year security support per branch. `edge` rolls continuously.                                       | Default for new projects. Small footprint, well-curated package set, all of yoe's tooling exercised against it. Picks up `module-alpine`'s ~12k main + community packages via passthrough; source-built userland from `module-core` links musl cleanly.                                                                                                                                                           |
| Debian | Experimental | New stable ~every 2 years; ~5-year support including LTS. `testing` and `unstable`/`sid` roll between releases.                          | Reach for it when an image needs glibc (CUDA, vendor drivers, enterprise software that hasn't been musl-ported), the broad apt ecosystem (debian main is ~50k packages), or compatibility with existing debian-based fleet management. End-to-end boot + SSH is not yet verified — treat any production deployment as untested. See [module-debian.md](module-debian.md) for current limitations and workarounds. |
| Ubuntu | Planned      | LTS every 2 years (April of even years), interim releases every 6 months; 5-year LTS support, 10 with Ubuntu Pro / ESM.                  | Not yet implemented — there is no `module-ubuntu` today. The future case: glibc / `.deb` like debian, but when you specifically need Ubuntu's commercial hardware enablement (e.g. NVIDIA Jetson L4T is Ubuntu-based), certified-hardware driver stacks, or an existing Ubuntu fleet. Would shim over `debian_feed()` with Ubuntu's keyring and suites — see [Adding a new distro](#adding-a-new-distro).            |
...
`debian_feed()`, `container()`, and a small `units/*-enable.star` companion
layer for services the maintainer wants exposed at boot. The internal Go support
