...
booted system sees — see [Security and Threat Model](security.md). A plain
shell `rm -rf build/` will fail with permission errors on those files.
`yoe clean` handles the cleanup correctly: it tries a host-side rm first
and, on EACCES, falls back to running the rm inside the build container
where it has the privilege to remove root- and service-user-owned files.
You don't need `sudo`.
