<p align="center">
  <a href="https://yoebuild.org/">
    <img src="yoe-logo-amber-on-black.svg" alt="Yoe" width="360">
  </a>
</p>
...
See [Comparisons](comparisons.md) for detailed analysis of how `[yoe]`
...
See [The `yoe` Tool](yoe-tool.md) for the full CLI reference,
[Unit & Configuration Format](metadata-format.md) for the unit and config
spec, and [Build Languages](build-languages.md) for the Starlark rationale.
...
- [AI Skills](ai-skills.md) — AI-driven workflows for unit creation, build
  debugging, security auditing, and more
- [The `yoe` Tool](yoe-tool.md) — CLI reference for building, imaging, and
  flashing
- [Unit & Configuration Format](metadata-format.md) — Starlark unit and
  configuration spec
- [Naming and Resolution](naming-and-resolution.md) — how modules, units,
  and dependencies are named, referenced, and resolved
- [File Templates](file-templates.md) — moving inline file content out of
...
- [Build Environment](build-environment.md) — bootstrap, host tools, and
  build isolation
- [Build Languages](build-languages.md) — analysis of Starlark, CUE, Nix,
  and other embeddable languages for unit definitions
- [Development Environments](dev-env.md) — the no-SDK model, `yoe shell`
  for interactive dev, and `yoe bundle` for air-gapped distribution
- [Testing](testing.md) — testing strategy across Go logic, package QA,
  image smoke tests, and on-device test runs
- [apk Signing](signing.md) — keypair generation, signature verification,
  and on-device trust
- [On-Device Package Management](on-device-apk.md) — using `apk` on booted
  yoe systems to install and upgrade packages
- [Feed Server and `yoe deploy`](feed-server.md) — dev-loop for serving the
  project apk repo and installing units on running devices
- [Containers on yoe Images](containers.md) — design for running Docker /
  Podman / containerd workloads on yoe-built devices
- [libc, init, and the Rootfs Base](libc-and-init.md) — the default base of
  musl, busybox, and OpenRC, and the path to glibc/systemd for edge-AI hardware
- [module-alpine](module-alpine.md) — wrapping prebuilt Alpine packages as
  yoe units
- [Comparisons](comparisons.md) — how `[yoe]` relates to Yocto, Buildroot,
  Alpine, Arch, and NixOS
- [Roadmap](roadmap.md) — existing units and what's needed for a complete
  base system
...
`[yoe]` is licensed under the [Apache License 2.0](https://github.com/yoebuild/yoe/blob/main/LICENSE).
